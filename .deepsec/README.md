...
Currently configured project: `aec-auth` (target: `..`).
...
original init command. Checkpoints in `data/aec-auth/setup/setup-state.json`
...
`--project-id aec-auth` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/aec-auth/`.
